...
public class GameButton {
    ...
    public void update(float dt) {
        ...
        vec.set(CBInput.x, CBInput.y, 0);
        cam.unproject(vec);

        if(text != null){
            if (CBInput.isPressed() &&
                    vec.x > x - width / 2  && vec.x < x - width / 2 + 38 &&
                    vec.y > y - height / 2 && vec.y < y - height / 2 + 38) {
                clicked = true;
            } else {
                clicked = false;
            }
    ...
    public void render(SpriteBatch sb) {
        ...
        if (text != null) {
            sb.draw(reg, x - width / 2, y - height / 2, 38, 38);
    ...
    private void drawString(SpriteBatch sb, String s, float x, float y) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c == '/') c = 10;
                ...
                sb.draw(font[c], x + i * 9 - xo - 14 , y - yo - 20, 12, 12);
        }
    }
